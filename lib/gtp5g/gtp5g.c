...
{
    int rt = 0;
pr_info("Enter gtp5g_buf_skb_ipv4\n");
                                ...
                                struct gtp5g_pktinfo *pktinfo)
{
    ...
    pr_info("Enter gtp5g_handle_skb_ipv4\n");
    ...
    if (far) {
        ...
        switch (far->action & FAR_ACTION_MASK) {
            ...
            case FAR_ACTION_BUFF:
                ...
                pr_info("Enter gtp5g_handle_skb_ipv4 -> gtp5g_buf_skb_ipv4\n");
...
{
    int rt = 0;
    pr_info("Enter gtp5g_buf_skb_encap\n");
    if (unix_sock_send(pdr, skb->data, skb_headlen(skb)) < 0)
        rt = -EBADMSG;

    dev_kfree_skb(skb);

    pr_info("Exit gtp5g_buf_skb_encap\n");
    return rt;
}
...
static int gtp5g_genl_del_pdr(struct sk_buff *skb, struct genl_info *info)
{
    ...
    // TODO: Use hash fuction to find the target PDR and delete it, but far will be delete
    rcu_read_lock();
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("free5GC UPF Team <free5gc.org@gmail.com>");
MODULE_DESCRIPTION("Interface driver for 5G GTP encapsulated traffic");
