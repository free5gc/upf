...
struct RtCbParams {
    int family;
    int table;
    ListNode *routeEntries; // list of RouteEntryNode
};
...
ListNode* KnetGetRoutes(int family, int table) {
    ListNode *routeEntries = UTLT_Malloc(sizeof(ListNode));
    ListInit(routeEntries);
...
void KnetRtListFree(ListNode* rtEntries) {
    ...
    node = ListFirst(rtEntries);
    while (node) {
        nextNode = (RouteEntryNode *)ListNext(node);
        ListRemove(rtEntries, node);
        UTLT_Free(node);
        node = nextNode;
    }
}
...
Status KnetPrintRoutes(ListNode *routeEntries) {
    ...
    for (RouteEntryNode *it = ListFirst(routeEntries); it != NULL; it = ListNext(it)) {
...
static int RtDataCallback(const struct nlmsghdr *nlh, void *data) {
    ...
    ListNode *routeEntries = callbackParams->routeEntries;
    RouteEntryNode *rtEntry = AllocRouteEntry();
    ListAppend(routeEntries, rtEntry);
