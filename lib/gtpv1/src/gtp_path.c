...
Status GtpServerListCreate(int epfd, int domain, ListNode *sockList, int port, 
                           SockHandler handler, void *data) {
    UTLT_Assert(sockList, return STATUS_ERROR, "Socket List is NULL");

    for (SockNode *it = ListFirst(sockList); it != NULL; it = ListNext(it)) {
...
Status GtpDevListCreate(int epfd, int domain, ListNode *sockList,
                        SockHandler handler, void *data) {
    UTLT_Assert(sockList, return STATUS_ERROR, "Socket List is NULL");
...
SockNode *GtpFindSockNodeByIp(ListNode *list, Ip *ip) {
    ...
    int check = 0;
    Sock *sockPtr = NULL;
    for (SockNode *it = ListFirst(list); it != NULL; it = ListNext(it)) {
        sockPtr = it->sock;
